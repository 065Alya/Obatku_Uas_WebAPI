...
@section('title', 'Tulis Artikel Baru — Admin')
...
@section('content')
    ...
    <div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 animate-fade-in-up">
        <div>
            ...
            <h1 class="text-2xl font-bold text-[#042C53]">Tulis Artikel Baru</h1>
        </div>
    </div>
        ...
        <form action="{{ route('admin.articles.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative">
                        <select id="category" name="category" class="obk-input appearance-none cursor-pointer" required>
                            ...
                            <option value="Tips Kesehatan" {{ old('category') === 'Tips Kesehatan' ? 'selected' : '' }}>Tips Kesehatan</option>
                            <option value="Panduan Obat" {{ old('category') === 'Panduan Obat' ? 'selected' : '' }}>Panduan Obat</option>
                            <option value="Gaya Hidup" {{ old('category') === 'Gaya Hidup' ? 'selected' : '' }}>Gaya Hidup</option>
                            <option value="Informasi Medis" {{ old('category') === 'Informasi Medis' ? 'selected' : '' }}>Informasi Medis</option>
                            <option value="EcoMed SDG 12" {{ old('category') === 'EcoMed SDG 12' ? 'selected' : '' }}>EcoMed (SDG 12)</option>
                        </select>
            ...
            <div>
                ...
                <textarea id="excerpt" 
                          ...
                          rows="2" 
                          class="obk-input" 
                          placeholder="Ringkasan singkat tentang isi artikel (maksimal 500 karakter)...">{{ old('excerpt') }}</textarea>
            </div>
                ...
                <div class="flex items-center gap-3">
                    <div class="relative flex items-start">
                        ...
                        <div class="ml-3 text-sm">
                            <label for="is_published" class="font-bold text-[#042C53]">Terbitkan Segera</label>
                            <p class="text-gray-500 text-xs">Jika dicentang, artikel akan langsung dapat dibaca oleh seluruh pengguna.</p>
                        </div>
                    </div>
                </div>
